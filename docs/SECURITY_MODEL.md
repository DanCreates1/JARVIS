...
Phase 11 proactivity preserves this authority boundary. User/model/research/API proposals are
...
only inert content-free candidate metadata. Phase 11B adds a separate default-off foreground runner
with atomic bounded leases, content-minimized local inbox/checkpoints, terminal feedback, and a
ten-attempt recovery ceiling. Its optional task handoff rechecks exact current host, rule, candidate,
task version/digest/status/deadline, and intersected budgets. Handoff never executes the task or
creates/binds approval. No daemon, provider/tool invocation, effect, external notification send,
cloud disclosure, or candidate-content retention is present. Immediate disable and transitive
